...
export class PostRepositoryImpl implements PostRepository {
    ...
    async updatePost(updatePostDto: UpdatePostDto): Promise<PostEntity> {
        ...
        if (updatePostDto.userId !== post.user.id.toString() && updatePostDto.user_role !== "admin") throw new Error("User not authorized to update this post.");
        const postUpdated = await this.postDataSource.updatePost(updatePostDto);
        if (!postUpdated) throw new Error("Error updating post.");
        return postUpdated;
    }

    async deletePost(id: string, userId: string, userRole: "editor" | "admin"): Promise<PostEntity> {
        const images = await this.postDataSource.getImagesFromPost(id);

        const post = await this.postDataSource.getPost(id);
        if (!post) throw new Error("Post not found.");

        if (userId !== post.user.id.toString() && userRole !== "admin") throw new Error("User not authorized to update this post.");
